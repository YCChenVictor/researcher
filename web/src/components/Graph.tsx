...
const createGraphRuntime = ({
  ...
  const simulation = d3
    ...
    .force(
      "link",
      d3.forceLink<Node, LinkSim>(links).id((d) => d.key).distance(120),
    )
  ...
  return {
    ...
    linkGroup,
    nodeGroup,
    linkSel: linkGroup.selectAll<SVGLineElement, LinkSim>("line"),
    nodeSel: nodeGroup.selectAll<SVGGElement, Node>("g.node"),
    drag,
    selectedSource: null,
  };
};
...
export const ForceGraph: React.FC = () => {
  ...
  const run = async () => {
    ...
    const simulationInstance = createSimulation(
      initial.nodes,
      initial.links,
      width,
      height,
    ...
    const drag = createDrag(simulationInstance);
    ...
    rt.simulation = simulationInstance;
    ...
    rt.simulation.on("tick", () => onTick(rt));
  };
